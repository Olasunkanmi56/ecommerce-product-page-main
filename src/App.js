...
import {FaChevronRight, FaShoppingCart, FaChevronLeft} from "react-icons/fa"
import {data} from "./data"
import minus from "./images/icon-minus.svg"
import plus from "./images/icon-plus.svg"
import Header from "./compos/Header";
import Lightbox from "./compos/LightBox";
...
function App() {
  ...
  return ( 
    <>
      <Header/>
       {showLightBox && <Lightbox setShowLightBox={setShowLightBox} products={products} prevSlide={prevSlide} slideIndex={slideIndex} nextSlide={nextSlide} />}
      <section  className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:py-20 place-items-center">
        <article>
          <div className="lg:hidden">
            ...
            <img src={item.mainImage} alt="product" className="w-full lg:w-[400px] lg:rounded-2xl cursor-pointer"
            onClick={() => setShowLightBox(true)} />
             ...
             {products.map((item, index) => (
              <div key={index} className={slideIndex === index + 1 ? "relative" : "hidden"}>
            <img src={mainImage} alt="product" className="w-full lg:w-[500px] lg:rounded-2xl cursor-pointer"
            onClick={() => setShowLightBox(true)} />
              <ul className="lg:hidden">
                <li><button onClick={prevSlide} className="bg-white rounded-full p-5 shadow absolute left-4 top-1/2 -translate-y-1/2"><FaChevronLeft /></button></li>
                <li><button onClick={nextSlide} className="bg-white rounded-full p-5 shadow absolute right-4 top-1/2 -translate-y-1/2"><FaChevronRight /></button></li>
            ...
            {products.map((item, index) => (
                  <li key={item.id} onClick={() => setValue(index)} className={`${
                    ...
                    <img src={item.thumbnail} alt="product" className="w-20" /> 
                    </li>
          ))}
            </ul>
        </article>
...
<div className="flex flex-wrap items-center justify-between lg:flex-col lg:items-start lg:gap-2">
  <ul className="flex items-center gap-5 mt-2">
    <li className="text-slate-900 font-bold text-2xl">$150.00</li>
    <li className="bg-slate-100 py-1 px-2 text-orange-400 tracking-wide text-sm font-bold inline-block rounded shadow">50%</li>
  </ul>
...
<div className="mt-10 lg:flex item-center justify-between gap-2">
  ...
  <div className="lg:flex-1">
   <button  className="flex items-center justify-center gap-4 md:mt-0 bg-orange-500 py-2 px-4 text-white font-bold rounded-lg shadow 
   mt-5 hover:bg-orange-600 w-full translate-all duration-200">
